...
for i = 1:MPC_vars.N
    ...
    [stage(i).Ck, stage(i).ug, stage(i).lg] = getInequalityConstraints(borders(max(i-1,1),:),MPC_vars,ModelParams);%%使用max可以节省if
    % bounds
...
function [eC, eL] = getErrors(pathinfo, theta_virt,x_phys,y_phys)
    ...
    % contouring and lag error estimates 论文中写错了
